...
def send_request(url: str, head: str, params: dict = None) -> requests.Response:
    """
...
def scrape_game_html(url: str, head: str) -> dict:
    """
    scrape a game's HTML page
    """
    scraped = {}
    log.debug(f"Scraping HTML page {url}")
    req = send_request(url, head)

    page = bs(req.text, 'html.parser')
    scraped["title"] = page.title.string.replace(' - Interactive Porn Game', '')
    scraped["description"] = page.find("div", class_="info").find("p").string
    scraped["scenes"] = [ {"image": tag['src']} for tag in page.find("div", class_="player").find_all("img")[1:] ]
...
def api_search_req(type_search, query, url):
    ...
    if api_request:
        ...
        if "games" in api_request_json:
            api_search = api_request_json["games"]
...
def api_search_id(scene_id, url):
    ...
    req = send_request(search_url, HEADERS)
    return req
...
def api_search_query(query, url):
    params = { 'q': query }
    req = send_request(url, HEADERS, params)
    return req
...
def match_result(api_scene, range_duration=60, single=False, clip_id: str=None):
    ...
    if SCENE_TITLE:
        ...
        match_ratio_title = 0
    if game_title and api_scene.get("title"):
        match_ratio_title_url = difflib.SequenceMatcher(
            None, game_title.lower(), api_scene["title"].lower()).ratio()
    else:
...
api2_url = "https://lifeselector.com/game/GetEpisodeDetailsInJson/gameId"
...
if "movie" not in sys.argv and "gallery" not in sys.argv:
    ...
    if SCENE_URL:
        ...
        try:
            ...
            log.info(f"game_description: {game_description}")
    ...
    if SEARCH_TITLE:
        ...
        if api_search:
            result_search = []
            log.debug(f"sceneByName, api_search: {api_search}")
